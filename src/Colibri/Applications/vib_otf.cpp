...
int main(int argc, char** argv) {
  ...
  try {
    ...
    parms.setParametersOTF(inputFileName);
    if (id == 0) {
      parms.printParametersOTF();
    }
    ...
    if (id == 0) {
      std::cout << "Number of SP Calculations needed to construct OTF-PES: " << pes->getNumSPsForVSCF() << std::endl;
